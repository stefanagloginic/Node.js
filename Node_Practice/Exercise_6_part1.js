...
var filterFiles = require('./Exercise_6_part2.js'); //only one anonymous func created in part2.js file & exported, thus filterFiles = that function
var directory = process.argv[2];
var extension = process.argv[3];

filterFiles(directory, extension, function(err, filtered_list){
  if(err) //if there isnt an error, err=null. if there is this if will execute.
  {
    console.error(err);
  }
  ...
  {
    for(var i=0; i < filtered_list.length; i++){ //no error, loop through the data = filtered_list
