...
var url = process.argv[2]; //obtain the url from the 1st arg

http.get(url, function callback(response){
	response.pipe(bl(function(err, data){ //bl helps with dealing with when data ends and appends the chunks of data together.

		if(err){ //if theres an error don't try to print the data
			return console.error(err); 
		}

		var data_as_string = data.toString(); // data is a Buffer obj...change to string

		console.log(data_as_string.length); //first print the number of characters
		console.log(data_as_string); //second print the actual data as a string
